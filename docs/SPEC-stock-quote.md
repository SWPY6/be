...
6. KIS 해외 실시간 무료 시세 신청이 완료된 상태로 운영한다. 그래서 국내·미국 모두 실시간이다.
...
| `priceTiming` | `REALTIME` / `DELAYED`. 국내·미국 모두 `REALTIME`(전제 6). 해외 실시간 신청 전이라면 미국을 `DELAYED`로 두는 규칙 한 줄만 바꾼다. "실시간"은 제공자 기준 지연 시세가 아니라는 뜻이고, 캐시 TTL만큼의 지연은 `priceAt`으로 드러난다 |
...
| 14 | 국내·미국 종목 모두 `REALTIME`으로 표시한다. | `국내_종목은_실시간으로_표시한다`, `미국_종목은_실시간으로_표시한다` |
